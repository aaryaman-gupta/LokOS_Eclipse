...
import appCLF.clfCons;
import appCLF.clfProfile;
...
public class lokosCLF{
	...
	public static ExtentTest testCLF = null;
	...
	public static int[][] clf_check = new int[2][58];
...
//		app.loginTest.sync();
		...
		Thread.sleep(1000);
		ExtentManager.addScreenShotsToTest("CLF Bookeeper Screen", test);
		
		navigationCLF.clfButton();
		

		xc.changeSheet("CLFs");

		int[] dimensions = xc.getRowCols("CLFs");
		int row = dimensions[0];

		for (int r = 1; r < row; r++) {
			xc.changeSheet("CLFs");
			try {
				if (!(xc.getCellString(r, clfCons.Type_0).equalsIgnoreCase("Submit")
						|| xc.getCellString(r, clfCons.Type_0).equalsIgnoreCase("Status")))
					testFlow = reports.createTest("[" + r + "] " + xc.getCellString(r, clfCons.CLF_Name_1)
							+ " (Flow: " + xc.getCellString(r, clfCons.Type_0) +"->"+xc.getCellString(r, clfCons.CLF_Name_1)+ ")");
				test.log(Status.INFO, "Flow number " + r + " begins");
				System.out.println("\nFlow number " + r + " begins");
				
				
				if (xc.getCellString(r, clfCons.Type_0).equalsIgnoreCase("New CLF")) {

					navigationCLF.navToBlock(row);
					navigationCLF.newCLF();

					String clf = xc.getCellString(r, clfCons.CLF_Name_1);
					System.out.println("\nFlow started for New VO " + clf);
					testCLF = testFlow.createNode(
							"New SHG: " + clf + "(" + xc.getCellString(r, clfCons.Flow_Type_0) + ")");
					System.out.println("________________________");
					//////////////////////////////////////////////
					int[] val = clfProfile.idSelect_CLF(r);
					//////////////////////////////////////////////
					int[][] zeroIni=new int[clf_check[0].length][clf_check[1].length];
					summary.display(clf_check, testCLF);
					clf_check=zeroIni;
					String resultF = "New CLF " + xc.getCellString(r, clfCons.CLF_Name_1) + " Fails: " + val[1]
							+ "/" + val[2];
					testCLF.log(Status.INFO, resultF);
...
//				else if (xc.getCellString(r, clfCons.Type_0).equalsIgnoreCase("Update SHG")) {
//
//					navigation.navToVillage(r);
//					navigation.existingSHG(r);
//					navigation.openSHGProfile(r);
//
//					String shg = xc.getCellString(r, clfCons.shgNameColNum);
//					testCLF = testFlow.createNode(
//							"Update SHG: " + shg + "(" + xc.getCellString(r, clfCons.flowTypeColNum) + ")");
//					System.out.println("\nFlow started for New SHG " + xc.getCellString(r, clfCons.shgNameColNum));
//					System.out.println("________________________");
//					//////////////////////////////////////////////
...
//					testCLF.log(Status.INFO, resultF);
//					testCLF.log(Status.INFO, resultP);
//					testFlow.log(Status.INFO, resultF);
...
//
//				}
//				else if (xc.getCellString(r, clfCons.typeColNum).equalsIgnoreCase("Submit")) {
//
//					boolean webProcess_flag = true;
//					try {
//
//						test.log(Status.INFO, "Starting Updation Process...");
//						testCLF = test.createNode("Upload Data + Web Flow");
...
//						if (s.contains("(0)")) {
//							testCLF.log(Status.FAIL, "Data not sufficient to upload");
...
//								testCLF.log(Status.PASS, "Record Added To Queue");
//								System.out.println("Record Added To Queue");
//								appdriver.quit();
//							} else {
//								ExtentManager.addScreenShotsToLogFail("Fail Submission Process" + r, testCLF);
//								testCLF.log(Status.FAIL, "||||Error->Record was not added to the Queue||||");
...
//						webProcess_flag=false;
//						ExtentManager.addScreenShotsToLogFail("Fail: Submission Process " + r, testCLF);
//						testCLF.log(Status.FAIL, "...error in Updation Process");
...
//				}
//				else if (xc.getCellString(r, clfCons.typeColNum).equalsIgnoreCase("Status")) {					
//					testCLF=test.createNode("Status Check");
//					++shg_row_counter;
//					testCLF.log(Status.INFO, "Starting status process...");
...
//					testCLF.log(Status.PASS, "Status: " + s);
//					System.out.println("Status: " + s);
//					ExtentManager.addScreenShotsToTest("Status "+r, test);
//					appdriver.findElementById(loginConstants.alertOK2).click();
//					appdriver.findElementById(loginConstants.syncHome).click();
//					System.out.println("^^^^^^^^^^^^^^^^^^^^^^^^^^^^");
//					testCLF.log(Status.INFO, "...status record processed.");
//					System.out.println("...status record processed.\n");
//					navigation.shgButton();
//				}
//				else if (xc.getCellString(r, clfCons.typeColNum).equalsIgnoreCase("SHG Meetings")) {
//
//					appdriver.findElementById("com.microware.cdfi:id/icBack").click();
//					Thread.sleep(1000);
//					System.out.println("\n-----SHG Meetings-----");
//					String shg = xc.getCellString(r, clfCons.shgNameColNum);
//					System.out.println("\nFlow started for SHG Meetings " + xc.getCellString(r, clfCons.shgNameColNum));
//					/////////////////////////
...
//					navigation.shgButton();
//
//				}else if (xc.getCellString(r, clfCons.typeColNum).equalsIgnoreCase("Web")) {
//					web.LoginTest.startWeb();
//				}
				...
				System.out.println("Login and Sync Complete");
				Thread.sleep(2000);
				ExtentManager.addScreenShotsToLogPass("CLF Entry Screen", test);
				navigationCLF.clfButton();
